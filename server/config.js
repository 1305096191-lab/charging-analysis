...
function validate() {
  const warnings = []
  if (!config.username) warnings.push('USERNAME 未配置')
  if (!config.password) warnings.push('PASSWORD 未配置')
  if (!config.bearerToken) warnings.push('BEARER_TOKEN 为空，系统将需要前端登录获取')
  if (warnings.length) {
    console.warn('[Config] 警告:', warnings.join('; '))
  }
}
